% Output
% Results = [Amp,xPos,yPos,sigmaX,sigmaY,background=0,X0,Y0,resnorm/sum(ccd_image(:))]
%%
function [Results] = LS_fittingVer2(image,threshold,windowSize,preview_flag)
epsilon1 =  10^-5;
epsilon2 =  10^-5;
maxIter  = 500;
...
Results= GaussFit(image,threshold,curvefitoptions,windowSize,preview_flag);
end

function fitresults = GaussFit(image,threshold,curvefitoptions,windowSize,preview_flag) % do the storm calculation
...
if isempty(points)==0
    ...
    for i=1:nrspots
        ...
        fitresults(i,:)=[amplitude,xPos,yPos,sigmaX,sigmaY,rotAngle,background,X0,Y0,res^0.5/sum(img(:))];
    end
